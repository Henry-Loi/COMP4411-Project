...
protected:
  ...
  vec3f color;
  vec3f
      distAttenConst; // (constant_attenuation_coeff,linear_attenuation_coeff,quadratic_attenuation_coeff)
};

class DirectionalLight : public Light {
public:
  DirectionalLight(Scene *scene, const vec3f &orien, const vec3f &color,
                   const vec3f &distAttenConst)
...
public:
  SpotLight(Scene *scene, const vec3f &pos, const vec3f &orien,
            const vec3f &color, const vec3f &distAttenConst)
      : Light(scene, color, distAttenConst) {}
  virtual vec3f shadowAttenuation(const vec3f &P) const;
  virtual double distanceAttenuation(const vec3f &P) const;
  virtual vec3f getColor(const vec3f &P) const;

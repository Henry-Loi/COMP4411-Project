...
public:
  ...
  vec3f globalToLocalCoords(const vec3f &v) { return inverse * v; }

  vec3f localToGlobalCoords(const vec3f &v) { return xform * v; }

  vec4f localToGlobalCoords(const vec4f &v) { return xform * v; }
...
public:
  TransformRoot() : TransformNode(NULL, mat4f()) {}
};

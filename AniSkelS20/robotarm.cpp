...
//   glTranslatef(0.0, 0.8, 0.0);                          // move to the top of
//   the base glRotatef(theta, 0.0, 1.0, 0.0);             // turn the
//   whole assembly around the y - axis.rotation_base(h1); // draw
//   the rotation base

//       glTranslatef(0.0, h1, 0.0);         // move to the top of
//   the base glRotatef(phi, 0.0, 0.0, 1.0); // rotate around
//     the z-axis for the lower arm
// 	glTranslatef( -0.1, 0.0, 0.4 );
//     lower_arm(h2); // draw
//     the lower arm

//         glTranslatef(0.0, h2, 0.0);              // move to the top of
//     the lower arm glRotatef(psi, 0.0, 0.0, 1.0); // rotate around
//     z-axis for the upper arm
// 	upper_arm(h3);							// draw
//     the upper arm
...
// void lower_arm(float h) { // draw the
//     lower arm setDiffuseColor(0.85, 0.75, 0.25);
//     setAmbientColor(0.95, 0.75, 0.25);
//     y_box(h);
// }

// void upper_arm(float h) { // draw the
//     upper arm setDiffuseColor(0.85, 0.75, 0.25);
//     setAmbientColor(0.95, 0.75, 0.25);
//     glPushMatrix();
...
//     controls[BASE_ROTATION] =
//         ModelerControl("base rotation (theta)", -180.0, 180.0, 0.1, 0.0);
//     controls[LOWER_TILT] =
//         ModelerControl("lower arm tilt (phi)", 15.0, 95.0, 0.1, 55.0);
//         controls[UPPER_TILT] = ModelerControl("upper arm tilt
//     (psi)", 0.0, 135.0, 0.1, 30.0 );
// 	controls[CLAW_ROTATION] = ModelerControl("claw rotation (cr)", -30.0,
// 180.0, 0.1, 0.0 );
//     controls[BASE_LENGTH] = ModelerControl("base height (h1)", 0.5, 10.0,
//     0.1, 0.8 ); controls[LOWER_LENGTH] = ModelerControl("lower arm length
//     (h2)", 1, 10.0, 0.1, 3.0 ); controls[UPPER_LENGTH] =
//     ModelerControl("upper arm length (h3)", 1, 10.0, 0.1, 2.5 );
//     controls[PARTICLE_COUNT] = ModelerControl("particle count (pc)",
//     0.0, 5.0, 0.1, 5.0 );
...
//     ModelerApplication::Instance()->Init(&createRobotArm, controls,
//     NUMCONTROLS);

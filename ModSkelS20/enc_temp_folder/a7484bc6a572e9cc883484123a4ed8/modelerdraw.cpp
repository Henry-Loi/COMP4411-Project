...
void drawStar(float size) {
    ...
    for (int i = 0; i < numTriangles; ++i) {
        ...
        float x2 = radiusInner * std::cos(theta + angle * 0.5f);
        float y2 = radiusInner * std::sin(theta + angle * 0.5f);
        float x3 = radiusOuter * std::cos(theta + angle);
        float y3 = radiusOuter * std::sin(theta + angle);
        drawTriangle(x1, y1, 0, x2, y1, 0, x1, y2, 0);

...
bool openRayFile(const char rayFileName[]) {
  ...
  if (mds->m_rayFile != NULL) {
    ...
    fprintf(
        mds->m_rayFile,
        "camera { fov=30; position=(0,0.8,5); direction=(0,-0.8,-5); }\n\n");
    fprintf(
        mds->m_rayFile,
...
void drawStar(float size,float thickness) {
    // Define the star vertices
     // Define the star parameters
    int numTriangles = 5;  // Number of triangles in the star
    float radiusOuter = 0.5f*size;  // Outer radius of the star
    float radiusInner = 0.25f*size;  // Inner radius of the star

    // Calculate the angle between each triangle
    float angle = 2.0f * M_PI / numTriangles;
    ...
    for (int i = 0; i < numTriangles; ++i) {
        ...
        // Calculate the vertex positions for the inner triangle
        float x1 = radiusInner * std::cos(theta);
        float y1 = radiusInner * std::sin(theta);
        float x2 = radiusInner * std::cos(theta + angle);
        float y2 = radiusInner * std::sin(theta + angle);

        // Calculate the vertex position for the outer triangle
        float x3 = radiusOuter * std::cos(theta + angle * 0.5f);
        float y3 = radiusOuter * std::sin(theta + angle * 0.5f);
        drawTriangle(x1, y1, 0, x2, y2, 0, x3, y3, 0);
        drawTriangle(x1, y1, 0, x2, y2, 0, 0, 0, 0);
        drawTriangle(x1, y1, thickness, x2, y2, thickness, x3, y3, thickness);
        drawTriangle(x1, y1, thickness, x2, y2, thickness, 0, 0, thickness);
        drawTriangle(x1, y1, thickness, x3, y3, thickness, x1, y1, 0);
        drawTriangle(x1, y1, 0, x3, y3, thickness, x3, y3, 0);
        drawTriangle(x2, y2, thickness, x3, y3, thickness, x2, y2, 0);
        drawTriangle(x2, y2, 0, x3, y3, thickness, x3, y3, 0);

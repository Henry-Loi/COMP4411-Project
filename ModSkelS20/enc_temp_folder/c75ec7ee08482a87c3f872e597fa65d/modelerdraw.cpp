...
void drawStar(float size) {
    ...
    int numTriangles = 8;  // Number of triangles in the star
    ...
    for (int i = 0; i < numTriangles; ++i) {
        ...
        // Calculate the vertex positions for the current triangle
        float x1 = radiusOuter * std::cos(theta);
        float y1 = radiusOuter * std::sin(theta);
        float x2 = radiusOuter * std::cos(theta + angle);
        float y2 = radiusOuter * std::sin(theta + angle);

        // Calculate the vertex position for the inner triangle
        float x3 = radiusInner * std::cos(theta + angle * 0.5f);
        float y3 = radiusInner * std::sin(theta + angle * 0.5f);
        drawTriangle(x1, y2, 0, x2, y3, 0, x3, y1, 0);

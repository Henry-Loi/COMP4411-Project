...
void animation(int* anim_index) {
    
    if(*anim_index < 45) {
        *anim_index++;
        SETVAL(FULL_MOVEMENT, 1);
        SETVAL(BODY_PITCH, -30 + *anim_index);
        glutPostRedisplay();
    }
    *anim_index = 0;
...
void animateCallback(void*) {
    ...
    Fl::add_timeout(0.01, animateCallback);
}
...
void RobotModel::draw() {
    ...
    if (VAL(SHOCKWAVE) == 1) {
        ...
        drawComplexShape(); //draw shockwave
...
int main() {
  ...
  controls[ANIMATION] =
      ModelerControl("Animation", 0, 1, 1, 0);
  m_app = ModelerApplication::Instance();
  Fl::add_timeout(0.01, animateCallback);

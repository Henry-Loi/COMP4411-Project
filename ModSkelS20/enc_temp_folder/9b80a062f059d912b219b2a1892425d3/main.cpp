...
void RobotModel::draw() {
  ...
  if (VAL(FULL_MOVEMENT) != 0) {
    sideLeftFeetangle = VAL(BODY_PITCH) / 2;
    sideRightFeetangle = -VAL(BODY_PITCH) / 2;
    frontLegangle = -VAL(BODY_PITCH);
    torsoAngle = VAL(BODY_PITCH);
  ...
  drawMetaBall();
  glTranslated(-4, 0, 0);
  //setDiffuseColorwithAlpha(0.2f, 0.9f, 1.0f, 0.6);

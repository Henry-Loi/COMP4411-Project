...
//	Distributed under the New BSD Licence. (See accompanying file
// License.txt or copy at 	http://www.paulsprojects.net/NewBSDLicense.txt)
//////////////////////////////////////////////////////////////////////////////////////////
    ...
    for (int j = 0; j < gridSize; j++) {
      for (int k = 0; k < gridSize; k++) {
        ...
        cubes[currentCube].vertices[1] =
            &vertices[(i * (gridSize + 1) + j) * (gridSize + 1) + k + 1];
        cubes[currentCube].vertices[2] =
            &vertices[(i * (gridSize + 1) + (j + 1)) * (gridSize + 1) + k + 1];
        cubes[currentCube].vertices[3] =
            &vertices[(i * (gridSize + 1) + (j + 1)) * (gridSize + 1) + k];
        cubes[currentCube].vertices[4] =
            &vertices[((i + 1) * (gridSize + 1) + j) * (gridSize + 1) + k];
        cubes[currentCube].vertices[5] =
            &vertices[((i + 1) * (gridSize + 1) + j) * (gridSize + 1) + k + 1];
        cubes[currentCube].vertices[6] =
            &vertices[((i + 1) * (gridSize + 1) + (j + 1)) * (gridSize + 1) +
                      k + 1];
...
void CUBE_GRID::DrawSurface(float threshold) {
  ...
  glBegin(GL_TRIANGLES);
  {
    ...
    for (int i = 0; i < numCubes; i++) {
      ...
      // look this value up in the edge table to see which edges to interpolate
      // along
      ...
      for (int currentEdge = 0; currentEdge < 12; currentEdge++) {
        if (usedEdges & 1 << currentEdge) {
          CUBE_GRID_VERTEX *v1 =
              cubes[i].vertices[verticesAtEndsOfEdges[currentEdge * 2]];
          CUBE_GRID_VERTEX *v2 =
              cubes[i].vertices[verticesAtEndsOfEdges[currentEdge * 2 + 1]];

          float delta = (threshold - v1->value) / (v2->value - v1->value);
          // edgeVertices[currentEdge].position=v1->position +
          // delta*(v2->position - v1->position);
          edgeVertices[currentEdge].position[0] =
              v1->position[0] + delta * (v2->position[0] - v1->position[0]);
          edgeVertices[currentEdge].position[1] =
              v1->position[1] + delta * (v2->position[1] - v1->position[1]);
          edgeVertices[currentEdge].position[2] =
              v1->position[2] + delta * (v2->position[2] - v1->position[2]);
          // edgeVertices[currentEdge].normal=v1->normal + delta*(v2->normal -
          // v1->normal);
          edgeVertices[currentEdge].normal[0] =
              v1->normal[0] + delta * (v2->normal[0] - v1->normal[0]);
          edgeVertices[currentEdge].normal[1] =
              v1->normal[1] + delta * (v2->normal[1] - v1->normal[1]);
          edgeVertices[currentEdge].normal[2] =
      ...
      for (int k = 0; triTable[cubeIndex][k] != -1; k += 3) {
        glNormal3fv(
            edgeVertices[triTable[cubeIndex][k + 0]].normal.getPointer());
        glVertex3fv(
            edgeVertices[triTable[cubeIndex][k + 0]].position.getPointer());

        glNormal3fv(
            edgeVertices[triTable[cubeIndex][k + 2]].normal.getPointer());
        glVertex3fv(
            edgeVertices[triTable[cubeIndex][k + 2]].position.getPointer());

        glNormal3fv(
            edgeVertices[triTable[cubeIndex][k + 1]].normal.getPointer());
        glVertex3fv(
            edgeVertices[triTable[cubeIndex][k + 1]].position.getPointer());

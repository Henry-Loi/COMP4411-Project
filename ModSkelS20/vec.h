...
public:
  ...
  Vec3<T> cross(const Vec3<T> &a) {
    return Vec3<T>(n[1] * a.n[2] - n[2] * a.n[1], n[2] * a.n[0] - n[0] * a.n[2],
                   n[0] * a.n[1] - n[1] * a.n[0]);
  }
...
template <class T> Vec<T> Vec<T>::cross(const Vec<T> &a, const Vec<T> &b) {
#ifdef _DEBUG
  if (a.numElements != 3 || b.numElements != 3)
    throw VectorSizeMismatch();
#endif
  n[0] = a.n[1] * b.n[2] - a.n[2] * b.n[1];
  n[1] = a.n[2] * b.n[0] - a.n[0] * b.n[2];
  n[2] = a.n[0] * b.n[1] - a.n[1] * b.n[0];
...
template <class T> istream &operator>>(istream &is, const Vec<T> &a) {
  return is;
}
...
template <class T> inline ostream &operator<<(ostream &os, const Vec3<T> &v) {
  return os << "[" << v[0] << " " << v[1] << " " << v[2] << "]";
}

template <class T> inline istream &operator>>(istream &is, Vec3<T> &v) {
  return is >> v.n[0] >> v.n[1] >> v.n[2];
}

template <class T> inline void swap(Vec3<T> &a, Vec3<T> &b) {
  Vec3<T> t(a);
  a = b;
  b = t;
}
...
template <class T> inline Vec3<T> minimum(const Vec3<T> &a, const Vec3<T> &b) {
  return Vec3<T>(minimum(a.n[0], b.n[0]), minimum(a.n[1], b.n[1]),
                 minimum(a.n[2], b.n[2]));
}

template <class T> inline Vec3<T> maximum(const Vec3<T> &a, const Vec3<T> &b) {
  return Vec3<T>(maximum(a.n[0], b.n[0]), maximum(a.n[1], b.n[1]),
                 maximum(a.n[2], b.n[2]));
}
...
template <class T> inline ostream &operator<<(ostream &os, const Vec4<T> &v) {
  return os << v.n[0] << " " << v.n[1] << " " << v.n[2] << " " << v.n[3];
}

template <class T> inline istream &operator>>(istream &is, Vec4<T> &v) {
  return is >> v.n[0] >> v.n[1] >> v.n[2] >> v.n[3];
}

template <class T> inline void swap(Vec4<T> &a, Vec4<T> &b) {
  Vec4<T> t(a);
  a = b;
  b = t;
}
...
template <class T> inline Vec4<T> minimum(const Vec4<T> &a, const Vec4<T> &b) {
  return Vec4<T>(minimum(a.n[0], b.n[0]), minimum(a.n[1], b.n[1]),
                 minimum(a.n[2], b.n[2]), minimum(a.n[3], b.n[3]));
}

template <class T> inline Vec4<T> maximum(const Vec4<T> &a, const Vec4<T> &b) {
  return Vec4<T>(maximum(a.n[0], b.n[0]), maximum(a.n[1], b.n[1]),
                 maximum(a.n[2], b.n[2]), maximum(a.n[3], b.n[3]));
}
